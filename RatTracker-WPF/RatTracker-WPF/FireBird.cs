...
namespace RatTracker_WPF
{
  ...
  public class FireBird : PropertyChangedBase
  {
    ...
    private readonly string _fbConStr = "User=SYSDBA;Password=masterkey;Database=" +
                                        Environment.GetFolderPath(Environment.SpecialFolder.Personal) +
                                        @"\RatTracker\EDDB.FDB;Dialect=3;Charset=UTF8;ServerType=1;";
    ...
    public void InitDB()
    {
      ...
      else
      {
        ...
        {
          Logger.Debug("Oops. Couldn't create database, probably because I got launched for Oauth. Doing nothing. " +
                       ex.Message);
          return;
        }
      }
    ...
    public void DropDB()
    {
      ...
      using (var dropTables = _con.CreateCommand())
      {
    ...
    public void CreateTables()
    {
      ...
      try
      {
        ...
        using (var createDomain = _con.CreateCommand())
        {
    ...
    public void CreateIndexes()
    {
      ...
      try
      {
        ...
        using (var createIndex = _con.CreateCommand())
        {
...
/*
    ...
    public void TestInserts()
    {
      ...
      using (var querySystem = _con.CreateCommand())
      {
        querySystem.CommandText = "SELECT * from eddb_systems LIMIT 5";
        using (var r = querySystem.ExecuteReader())
        {
          while (r.Read())
          {
    ...
    {
      var systemResult = new List<EdsmSystem>();
      Status = "Working...";
      using (var getSystem = _con.CreateCommand())
      {
        getSystem.CommandText = "SELECT FIRST 50 name,id,x,y,z FROM eddb_systems WHERE lowercase_name LIKE '%" +
                                systemname.ToLower() + "%'";
        using (var r = await getSystem.ExecuteReaderAsync())
        {
          while (r.Read())
          {
            ...
            Logger.Debug("GetSystemEDSM added: " + r.GetString(0) + ": " + r.GetString(1) + " X: " + r.GetString(2) +
                         " Y: " + r.GetString(3) + " Z: " + r.GetString(4));
          }
        }
      }
      Status = "Ready!";
    ...
    public int GetSystemCount()
    {
      ...
      try
      {
        using (var querySystemCount = _con.CreateCommand())
        {
          querySystemCount.CommandText = "SELECT COUNT(id) from eddb_systems";
          using (var r = querySystemCount.ExecuteReader())
          {
            while (r.Read())
            {
              Status = "Ready!";
